...
dev_sentence_texts = dict(
    (
        idx,
        remove_puntuation(
            pyarabic.araby.strip_diacritics(
                validated_sentences.loc[validated_sentences["sentence_id"] == idx]["sentence"].item()
            )
        ),
    )
    for idx in dev_sentence_ids
)
test_sentence_texts = dict(
    (
        idx,
        remove_puntuation(
            pyarabic.araby.strip_diacritics(
    ...
    if text in dev_sentence_texts.values() or text in test_sentence_texts.values():
        ...
        for dev_id in dev_ids:
            if dev_id != val_sentence_id:
                ...
                print(
                    "val", validated_sentences[validated_sentences["sentence_id"] == val_sentence_id]["sentence"].item()
                )
        ...
        for test_id in test_ids:
            if test_id != val_sentence_id:
                print(
                    "test", str(validated_sentences[validated_sentences["sentence_id"] == test_id]["sentence"].item())
                )
                print(
                    "vald",
                    str(validated_sentences[validated_sentences["sentence_id"] == val_sentence_id]["sentence"].item()),
                )
...
validated_csv = validated_csv[validated_csv["sentence_id"].isin(unique_val_sentence_ids)]
print(len(validated_csv))

validated_csv.to_csv(os.path.join(dataset_root_dir, f"train_from_{dataset_split}_s.tsv"), sep="\t")
